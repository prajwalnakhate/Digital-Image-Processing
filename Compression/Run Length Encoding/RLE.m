%% Run Lenght Encoding using MATLAB
%Date: 7 April 2020
%Prajwal Nakhate
...
for i=1:length(x)-1
    if(x(i)==x(i+1)) 
        ...
        y=[y,count,x(i),];
        % else add elements to output
        %and check for next bit
    count=1; %reset count 
    end
end
%% Output 
y=[y,count,x(length(x))];
%to avoid index exceedning 
%adding last element seperately
%% Compression Ratio
lengthip=length(x);
lengthop=length(y);
z=lengthip/lengthop;
